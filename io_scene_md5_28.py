bl_info = {
    ...
    "author": "nemyax, 2.8 Update: Samson, 4.5 Update: Claude",
    "version": (1, 12, 20250125),
    "blender": (4, 0, 0),
...
def do_joints(md5mesh, j_re, e_re, correctMatrix,meshName,collection,boneLayer):
    ...
    bpy.ops.object.mode_set()
    bl = boneLayer
    # Updated for Blender 4.5 - collections replaced layers
    if hasattr(arm, 'collections'):
        # Create bone collection if it doesn't exist
        bone_collection_name = f"MD5_Layer_{bl}"
        if bone_collection_name not in arm.collections:
            bone_collection = arm.collections.new(bone_collection_name)
        else:
            bone_collection = arm.collections[bone_collection_name]
...
def bone_tree_blender(arm):
    ...
    # Updated for Blender 4.5 bone collections
    if hasattr(arm, 'collections'):
        bone_collection_name = f"MD5_Layer_{bl}"
        if bone_collection_name in arm.collections:
            bone_collection = arm.collections[bone_collection_name]
            # Fixed: Check by bone name, not bone object
            bones = [b for b in arm.bones if b.name in bone_collection.bones]
        else:
            bones = list(arm.bones)  # Use all bones if collection doesn't exist
    else:
        # Fallback for older versions
        bones = [b for b in arm.bones if hasattr(b, 'layers') and b.layers[bl]]
    return btb(None, bones)
...
def is_export_go(what, collection):
    ...
    # Updated bone selection for Blender 4.5
    if hasattr(armature.data, 'collections'):
        bone_collection_name = f"MD5_Layer_{bl}"
        if bone_collection_name in armature.data.collections:
            bone_collection = armature.data.collections[bone_collection_name]
            # Fixed: Get bones by name, not object comparison
            bones = [b for b in armature.data.bones if b.name in bone_collection.bones]
        else:
            bones = list(armature.data.bones)  # Use all bones if collection doesn't exist
    else:
        # Fallback for older versions
        bones = [b for b in armature.data.bones if hasattr(b, 'layers') and b.layers[bl]]
...
def manage_bone_layers(doWhat):
    ...
    if hasattr(armature, 'collections'):
        bone_collection_name = f"MD5_Layer_{bl}"
        if bone_collection_name not in armature.collections:
            bone_collection = armature.collections.new(bone_collection_name)
        else:
            bone_collection = armature.collections[bone_collection_name]
        
        if doWhat == 'replace':
            # Clear collection first
            for bone_name in list(bone_collection.bones.keys()):
                bone_obj = armature.bones[bone_name]
                bone_collection.unassign(bone_obj)
            # Add selected bones
            for x in selBones:
                bone_collection.assign(x)
        elif doWhat == 'add':
            for x in selBones:
                bone_collection.assign(x)
        elif doWhat == 'remove':
            for x in selBones:
                if x.name in bone_collection.bones:
                    bone_collection.unassign(x)
        elif doWhat == 'clear':
            for bone_name in list(bone_collection.bones.keys()):
                bone_obj = armature.bones[bone_name]
                bone_collection.unassign(bone_obj)
    else:
        # Fallback for older versions with layers
        if doWhat == 'replace':
            for x in selBones:
                if hasattr(x, 'layers'):
                    x.layers[bl] = True
...
def delete_action(prepend,actionNameToDelete):
    ...
    # Fixed: Check if animation_data exists and has an action
    if (armature.animation_data and 
        armature.animation_data.action and 
        armature.animation_data.action.name == actionName):
